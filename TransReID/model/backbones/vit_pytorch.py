...
class Cross_Attention(nn.Module):
    ...
    def forward(self, x, text_features=None):
        ...
        B, N, C = x.shape # 图像特征作为query

        kv = self.kv(text_features).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]  # Unpack key and value
        # Use provided text features as query
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        ############### zwq 
        # change to corss_attention class
        # self.qq = nn.Linear(dim, dim, bias=qkv_bias)
        # self.kv = nn.Linear(dim, dim * 2, bias=qkv_bias)  # Only compute key and value

        # self.linear = nn.Linear(24, 129)
        ##############################
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
...
class Block_MAPLE(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 design_details = None, text_layer=False, i=0): # MAPLE 
        super().__init__()
        self.norm1 = norm_layer(dim)
        ...
        #######3 MaPLe
        self.compound_prompt_nctx = design_details['maple_length'] # 提示词长度 a photo of(3) 
        if i == 0:
            self.first_layer = True
        else:
            self.first_layer = False

    def forward(self, input, text_feat =None):
        # input输入为列表[img, prompt, 计数器int]
        x = input[0]
        compound_prompts_deeper = input[1] # 提示此列表，每个元素为一个提示词
        counter = input[2]
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0: # 如果有提示词 text和visual的提示词不同，前向传播函数相同
                if not (counter > len(compound_prompts_deeper)-1):# 如果计数器小于提示词长度
                    prefix = x[:, 0:x.shape[1]-self.compound_prompt_nctx, :] # B*Seq*Dim 除去提示词的部分 

                    ############ 当前Layer可学习Tokensc
                    visual_context = compound_prompts_deeper[counter] # 提示词
                    visual_context = visual_context.expand(x.shape[0], -1, -1).half() #expand共享内存
                    x = torch.cat([prefix, visual_context], dim =1) # # 拼接图像特征和视觉提示
                    counter += 1
        x = x + self.drop_path(self.attn(self.norm1(x)))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class  TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id):
        ...
        # elif self.adain or self.text: # 如果要使用adain和clip，输出特征是全部token
    ...
    def load_param(self, model_path):
        ...
        # if "prompt_learner.token_prefix" in param_dict:
        #     del param_dict["prompt_learner.token_prefix"]

        # if "prompt_learner.token_suffix" in param_dict:
        #     del param_dict["prompt_learner.token_suffix"]
...
def vit_base_patch16_224_TransReID_Prompt(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    model = TransReid_Prompts(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,\
...
class TransReid_Prompts(nn.Module): ##### image encoder
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        ###########zwq
        self.adain = adain_norm
        ######## category instance normalization CSA
        self.csa = csa  
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed_overlap(
                img_size=img_size, patch_size=patch_size, stride_size=stride_size, in_chans=in_chans,
                embed_dim=embed_dim) # 3->768 conv2d
        ...
        self.blocks = nn.ModuleList([
            Block_MAPLE(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                design_details=design_details, text_layer=text_layer, i=i)
            for i in range(depth)])
    ...
    def forward_features(self, x, camera_id, shared_ctx, compound_deeper_prompts):
        ...
        x = self.pos_drop(x) ## 位置编码
    
        # MaPLe
        if self.VPT_shallow:
            visual_ctx = shared_ctx.expand(x.shape[0], -1, -1).half() # 256,2,768
            x = torch.cat([x, visual_ctx], dim=1) # 256,131,768
        else:
    ...
    def forward(self, x, cam_label,  shared_ctx, compound_deeper_prompts):
        x = self.forward_features(x, cam_label, shared_ctx, compound_deeper_prompts)
        return x
