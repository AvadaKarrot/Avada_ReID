...
if __name__ == '__main__':
    ...
    gallery_loader = data_manager.build_source_eval_loader()
    ...
    for n_iter, (img, _, _, _, img_path, captions) in enumerate(gallery_loader):
        ...
        for i in range(len(grayscale_cam)):
            ...
            # Reload untouched source pixels instead of visualizing the
            # resized and normalized tensor consumed by the model.
            raw_img = cv2.imread(img_path[i])
            if raw_img is None:
                logger.warning('Unable to read source image: %s', img_path[i])
                continue
            image_height, image_width = raw_img.shape[:2]
            display_img = np.float32(raw_img) / 255
            save_path = osp.join(cfg.OUTPUT_DIR, osp.basename(img_path[i]))

            grayscale_cam_i = cv2.resize(
                grayscale_cam[i, :],
                (image_width, image_height),
            )
            cam_image = show_cam_on_image(display_img, grayscale_cam_i)
