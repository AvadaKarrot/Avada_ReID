...
from data.sobel_filter import SobelFilter, SobelFilter_sep_Add
...
def build_transforms(
    ...
    randomerase_prob=0.5,###################### for random erase
    padding = 10, ############################## for random padding
    sobel_prob = 0.8, ############################
    **kwargs
):
    ...
    normalize = T.Normalize(mean=norm_mean, std=norm_std) ########### initialize normalize

    print('Building train transforms ...')
    transform_tr = []

    print('+ resize to {}x{}'.format(height, width))
    # transform_tr += [T.Resize((height, width), interpolation=3)]
    transform_tr += [T.Resize((height, width), interpolation=T.InterpolationMode.BICUBIC)]

    if 'random_flip' in transforms:
        print('+ random flip')
        transform_tr += [T.RandomHorizontalFlip(p=randomerase_prob)]

    if 'pad' in transforms:
        print('+ random padding')
        transform_tr += [T.Pad(padding)]
    ...
    if 'random_crop' in transforms:
        ...
        transform_tr += [T.RandomCrop([height, width])]
    ...
    if 'sobel' in transforms:
        print('+ sobel')
        transform_tr += [SobelFilter_sep_Add(ksize=3, alpha=sobel_prob)]

    if 'bilateral' in transforms:
        print('+ bilateral')
        transform_tr += [BilateralFilter(d=9, sigmaColor=75, sigmaSpace=75)]
    ...
    if 'random_erase' in transforms:
        print('+ random erase')
        transform_tr += [RandomErasing(probability=randomerase_prob, mode='pixel', max_count=1, device='cpu')]
