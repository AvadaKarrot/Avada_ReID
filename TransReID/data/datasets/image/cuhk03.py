...
def _rebase_split_paths(records, image_dir):
    """Make generated CUHK03 split files portable across machines."""
    return [
        (osp.join(image_dir, osp.basename(record[0])), *record[1:])
    ...
    def __init__(
        ...
        **kwargs
        ...
        if cuhk03_labeled:
            ...
            image_dir = self.imgs_labeled_dir
        else:
            split_path = self.split_classic_det_json_path if cuhk03_classic_split else self.split_new_det_json_path
            image_dir = self.imgs_detected_dir
        ...
        # Split JSON files generated by older torchreid versions contain
        # machine-specific absolute paths. Rebase them onto the active
        # dataset root so the same protocol files work after migration.
        train = _rebase_split_paths(split['train'], image_dir)
        query = _rebase_split_paths(split['query'], image_dir)
        gallery = _rebase_split_paths(split['gallery'], image_dir)
